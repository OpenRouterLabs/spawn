...
export function buildExportScript(opts: {
  ...
  return [
    ...
    "# Secret patterns grouped by provider family:",
    "# - OpenRouter: sk-or- prefix (v1, v2+, non-hex chars)",
    "# - Anthropic: sk-ant-api prefix",
    "# - OpenAI: sk-proj- prefix",
    "# - GitHub: gho/ghp/ghs tokens",
    "# - AWS: AKIA access key IDs",
    "# - Hetzner: hcloud_ tokens",
    "# - DigitalOcean: dop_v1_ tokens",
    "# - Slack: xoxb/xoxp/xoxa bot/user/app tokens",
    "# - Stripe: sk_live_ secret keys",
    "# - Discord: bot tokens (base64-encoded user ID.timestamp.HMAC)",
    "# - Google: service account JSON blocks",
    "# - PEM: private key blocks",
    'SECRET_REGEX=\'(sk-or-[a-zA-Z0-9_-]{20,})|(sk-ant-api[0-9-]+_[A-Za-z0-9_-]{20,})|(sk-proj-[A-Za-z0-9_-]{20,})|(gh[ops]_[A-Za-z0-9]{36})|(AKIA[0-9A-Z]{16})|(hcloud_[a-zA-Z0-9_-]{20,})|(dop_v1_[a-f0-9]{32,})|(xox[abp]-[0-9A-Za-z-]{10,})|(sk_live_[A-Za-z0-9]{24,})|([A-Za-z0-9_-]{24}\\.[A-Za-z0-9_-]{6}\\.[A-Za-z0-9_-]{27,})|("type":\\s*"service_account")|(-----BEGIN ([A-Z]+ )?PRIVATE KEY-----)\'',
