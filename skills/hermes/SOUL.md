...
`--headless` only provisions. To run a prompt on the child: `ssh -i ~/.ssh/spawn_ed25519 root@<ip> "bash -lc 'claude -p --dangerously-skip-permissions \"prompt\"'"`. Always use `bash -lc` (binaries are in ~/.local/bin/).
